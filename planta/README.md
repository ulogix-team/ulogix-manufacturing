<p align="center">
  <img src="https://raw.githubusercontent.com/ulogix-team/assets/main/dividers/divider-section.svg" width="100%"/>
</p>

# 🏭 Planta — Diseño de Distribución

Diseño de la distribución física de la planta y planos de layout.

## Contenido esperado

- **layout/** — Planos 2D de distribución en planta (Draw.io / AutoCAD / NX)
- **calculos/** — Hojas de cálculo de dimensionamiento, cálculo de áreas, flujos

## Responsable

Juan M. Beltrán · [@JuanBeltran2024](https://github.com/JuanBeltran2024)
